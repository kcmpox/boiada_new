  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !value && onBack()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
        ...
        <DialogFooter><Button type="button" variant="outline" onClick={() => { const blob = new Blob([JSON.stringify(jsonValue, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `recebimento-${paymentDate}.json`; link.click(); URL.revokeObjectURL(url); toast.success("Registro baixado") }}><Download data-icon="inline-start" /> Baixar registro</Button><Button type="button" variant="outline" size="icon" aria-label="Exibir JSON do recebimento" title="Exibir JSON" onClick={() => setJsonOpen(true)}><Code2 /></Button><Button type="button" onClick={saveReceipt}><Save data-icon="inline-start" /> Salvar recebimento</Button></DialogFooter>
      </DialogContent>
