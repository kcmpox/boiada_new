...
import { useSlaughterhouses, useTrips, useTrucks, useFuelings, useExpenses, useDeductions, useReimbursements, usePayments } from "@/lib/storage";
...
export function AlternativeLayoutDialog({ open, title, onBack }: { open: boolean; title: string; onBack: () => void }) {
  ...
  const fuelingItems = useMemo(() => fuelings.flatMap((fueling) => fueling.items.map((item, index) => ({ id: `${fueling.id}:${index}`, fuelingId: fueling.id, date: fueling.date, truckId: fueling.truckId, description: item.description, amount: Math.max(0, item.quantity * item.unitPrice - (item.discount || 0)), responsibility: item.responsibility ?? fueling.responsibility }))), [fuelings])
  const receivedTotal = Object.values(receivedByItem).reduce((sum, value) => sum + (Number(value.replace(",", ".")) || 0), 0)
  ...
  const selectedExpenses = expenses.filter((item) => selectedIds.includes(item.id) && item.responsibility !== "minha")
  const selectedDeductions = deductions.filter((item) => selectedIds.includes(item.id))
  const selectedReimbursements = reimbursements.filter((item) => selectedIds.includes(item.id))
  const grossValue = selectedTrips.reduce((total, trip) => total + (trip.tableValue ?? trip.finalValue ?? 0), 0)
  const fuelingsValue = selectedFuelingItems.reduce((total, item) => total + item.amount, 0)
  ...
  const expectedValue = (grossValue * 0.9) - fuelingsValue - deductionsValue + reimbursementsValue
  const ready = Boolean(truckId && destination)
  const jsonValue = { date: paymentDate, truckId, destination, tripIds: selectedTrips.map((trip) => trip.id), trips: selectedTrips.map((trip) => ({ ...trip, cte: tripEdits[trip.id]?.cte ?? trip.cte, minuta: tripEdits[trip.id]?.minuta ?? trip.minuta })), fuelingItemIds: selectedFuelingItems.map((item) => item.id), fuelings: selectedFuelingItems, expenseIds: selectedExpenses.map((item) => item.id), expenses: selectedExpenses, deductionIds: selectedDeductions.map((item) => item.id), deductions: selectedDeductions, reimbursementIds: selectedReimbursements.map((item) => item.id), reimbursements: selectedReimbursements, grossValue, rentValue, reimbursedValue: selectedReimbursements.reduce((sum, item) => sum + item.amount, 0), deductedValue: selectedDeductions.reduce((sum, item) => sum + item.amount, 0), expectedValue, fuelingsValue, deductionsValue, reimbursementsValue, receivedValue: receivedTotal || Number(receivedValue.replace(",", ".")) || 0, receivedByItem, notes }

  const saveReceipt = () => {
    setPayments((current) => [{ id: crypto.randomUUID(), date: paymentDate, destination: destination as any, tripIds: selectedTrips.map((trip) => trip.id), fuelingIds: [], expenseIds: selectedExpenses.map((item) => item.id), tollIds: [], rentPercent: 0.1, grossValue, rentValue, reimbursedValue: selectedReimbursements.reduce((sum, item) => sum + item.amount, 0), deductedValue: selectedDeductions.reduce((sum, item) => sum + item.amount, 0), expectedValue: jsonValue.expectedValue, receivedValue: jsonValue.receivedValue, fuelingItemIds: selectedFuelingItems.map((item) => item.id), notes } as any, ...current]);
    ...
    <Dialog open={open} onOpenChange={(value) => !value && onBack()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
        ...
        <div className="flex flex-col gap-4">
          ...
          {ready && <Card><CardHeader><div className="flex flex-wrap gap-2">{tabs.map((tab) => <Button key={tab} type="button" size="sm" variant={activeTab === tab ? "secondary" : "ghost"} onClick={() => setActiveTab(tab)}>{tab}</Button>)}</div></CardHeader><CardContent>{activeTab !== "Viagens" ? <div className="space-y-3">{(activeTab === "Abastecimentos" ? fuelingItems.filter((item) => item.responsibility !== "minha") : activeTab === "Manutenções" ? expenses.filter((item) => item.responsibility !== "minha") : activeTab === "Descontos" ? deductions : activeTab === "Reembolsos" ? reimbursements : []).filter((item) => "truckId" in item && item.truckId === truckId).map((item) => <div key={item.id} className="flex w-full items-start gap-3 rounded-xl border px-3 py-3 text-sm"><Checkbox className="mt-1" checked={selectedIds.includes(item.id)} onCheckedChange={() => toggleTrip(item.id)} /><div className="min-w-0 flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="truncate font-medium">{"description" in item ? item.description : "Lançamento financeiro"}</p><p className="truncate text-xs text-muted-foreground">{String(item.date ?? "").slice(0, 10)} • {trucks.find((truck) => truck.id === item.truckId)?.plate ?? "Caminhão"} • {"fuelingId" in item ? `Abastecimento ${item.fuelingId}` : "Lançamento financeiro"}</p></div><div className="shrink-0 text-left sm:text-right"><p className="text-xs text-muted-foreground">Valor item</p><p className="font-semibold">{money(Number("value" in item ? item.value : item.amount) || 0)}</p><p className="font-semibold text-primary">{money(Number(receivedByItem[item.id]?.replace(",", ".")) || ("value" in item ? item.value : item.amount))}</p></div></div><div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>Quantidade: {"quantity" in item ? item.quantity : "—"}</span><span>Preço unitário: {"unitPrice" in item ? money(item.unitPrice) : "—"}</span>{"responsibility" in item && item.responsibility && <span>Responsável: {item.responsibility}</span>}</div>{selectedIds.includes(item.id) && <div className="mt-3 flex items-center gap-2 border-t pt-3"><Label htmlFor={`received-${item.id}`} className="text-xs">Valor recebido</Label><Input id={`received-${item.id}`} className="w-36" inputMode="decimal" placeholder={money(item.amount)} value={receivedByItem[item.id] ?? ""} onChange={(event) => setReceivedByItem((current) => ({ ...current, [item.id]: event.target.value }))} /></div>}</div></div>)}{(activeTab === "Pedágios" || (activeTab !== "Viagens" && !["Abastecimentos", "Manutenções", "Descontos", "Reembolsos"].includes(activeTab))) && <p className="py-8 text-center text-sm text-muted-foreground">Esta sessão está em construção</p>}</div> : <div className="space-y-3">{filteredTrips.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">{dateFrom ? "Nenhuma viagem encontrada no período." : "Informe uma data inicial ou selecione as viagens disponíveis até a data do pagamento."}</p> : filteredTrips.map((trip) => { const edit = tripEdits[trip.id] ?? { cte: trip.cte ?? "", minuta: trip.minuta ?? "" }; const bruto = trip.tableValue ?? trip.finalValue ?? 0; const liquido = bruto * 0.9; return <div key={trip.id} className="flex items-center gap-3 rounded-xl border px-3 py-3 text-sm"><div className="flex w-full items-start gap-3"><Checkbox className="mt-1" checked={selectedIds.includes(trip.id)} onCheckedChange={() => toggleTrip(trip.id)} />{selectedIds.includes(trip.id) && <Input className="w-32 shrink-0" inputMode="decimal" aria-label={`Valor recebido da viagem ${trip.id}`} placeholder={money(liquido)} value={receivedByItem[trip.id] ?? ""} onChange={(event) => setReceivedByItem((current) => ({ ...current, [trip.id]: event.target.value }))} />}<div className="min-w-0 flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="truncate font-medium">{trip.origin} → {slaughterhouses.find((item) => item.id === trip.destination)?.name ?? trip.destination}</p><p className="truncate text-xs text-muted-foreground">{trip.date} • {trucks.find((truck) => truck.id === trip.truckId)?.plate ?? "Caminhão"} • {trip.distance ?? 0} km</p></div><div className="shrink-0 text-left sm:text-right"><p className="text-xs text-muted-foreground">Valor viagem</p><p className="font-semibold">{money(bruto)}</p><p className="font-semibold text-primary">{money(liquido)}</p></div></div><div className="mt-2 flex flex-wrap items-center gap-2 text-xs">{edit.cte ? <button type="button" className="text-primary underline-offset-2 hover:underline" onClick={() => { const value = window.prompt("Editar CTe", edit.cte); if (value !== null) editTrip(trip.id, "cte", value) }}>CTe: {edit.cte}</button> : <button type="button" className="text-primary underline-offset-2 hover:underline" onClick={() => { const value = window.prompt("Informar CTe", ""); if (value !== null) editTrip(trip.id, "cte", value) }}>Adicionar CTe</button>}<span>Bruto: {money(bruto)}</span>{edit.minuta ? <button type="button" className="text-primary underline-offset-2 hover:underline" onClick={() => { const value = window.prompt("Editar Minuta", edit.minuta); if (value !== null) editTrip(trip.id, "minuta", value) }}>Minuta: {edit.minuta}</button> : null}</div></div></div></div> })}</div>}</CardContent></Card>}
          <Card><CardContent className="flex flex-col gap-4 pt-6"><div className="grid gap-3 sm:grid-cols-6"><div><p className="text-sm text-muted-foreground">Valor bruto</p><p className="font-semibold">{money(grossValue)}</p></div><div><p className="text-sm text-muted-foreground">Aluguel de 10%</p><p className="font-semibold text-destructive">- {money(rentValue)}</p></div><div><p className="text-sm text-muted-foreground">Combustíveis</p><p className="font-semibold text-destructive">- {money(fuelingsValue)}</p></div><div><p className="text-sm text-muted-foreground">Descontos e Reembolsos</p><p className="font-semibold">{money(reimbursementsValue - deductionsValue)}</p></div><div className="sm:col-span-2"><p className="text-sm text-muted-foreground">Valor esperado</p><p className="text-lg font-bold">{money(expectedValue)}</p></div></div><Separator /><div className="grid gap-4 sm:grid-cols-2"><div><Label htmlFor="received-value">Valor recebido neste dia</Label><Input id="received-value" inputMode="decimal" placeholder="Se vazio, usa o valor líquido" value={receivedValue} onChange={(event) => setReceivedValue(event.target.value)} /></div><div><Label htmlFor="payment-notes">Observação</Label><Textarea id="payment-notes" rows={2} placeholder="Opcional" value={notes} onChange={(event) => setNotes(event.target.value)} /></div></div></CardContent></Card>
        </div>
