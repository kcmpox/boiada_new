    ...
    <Dialog open={open} onOpenChange={(value) => !value && onBack()}>
      <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-4xl">
        ...
        <div className="flex flex-col gap-4">
          ...
          {ready && <Card><CardHeader><div className="flex flex-wrap gap-2">{tabs.map((tab) => <Button key={tab} type="button" size="sm" variant={activeTab === tab ? "secondary" : "ghost"} onClick={() => setActiveTab(tab)}>{tab}</Button>)}</div></CardHeader><CardContent>{activeTab !== "Viagens" ? <p className="py-8 text-center text-sm text-muted-foreground">Esta sessão está em construção</p> : <div className="space-y-3">{filteredTrips.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">Nenhuma viagem encontrada no período.</p> : filteredTrips.map((trip) => { const edit = tripEdits[trip.id] ?? { cte: trip.cte ?? "", minuta: trip.minuta ?? "" }; const bruto = trip.tableValue ?? trip.finalValue ?? 0; const liquido = trip.finalValue ?? bruto; return <div key={trip.id} className="rounded-xl border bg-card p-4 shadow-sm transition-colors hover:bg-muted/20"><div className="flex items-start gap-3"><Checkbox className="mt-1" checked={selectedIds.includes(trip.id)} onCheckedChange={() => toggleTrip(trip.id)} /><div className="min-w-0 flex-1"><div className="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><p className="truncate font-semibold">{trip.date} · {trip.origin} → {trip.destination}</p><p className="truncate text-xs text-muted-foreground">{trip.date} • {trip.origin} • {trip.destination} • {trip.distance ?? 0} km</p></div><div className="shrink-0 text-left sm:text-right"><p className="text-xs text-muted-foreground">Líquido</p><p className="font-semibold text-primary">{money(liquido)}</p></div></div><div className="mt-2 flex flex-wrap items-center gap-2 text-xs"><span className="rounded-full bg-muted px-2 py-1 text-muted-foreground">Bruto: {money(bruto)}</span>{edit.cte && <span className="rounded-full bg-primary/10 px-2 py-1 text-primary">CTe: {edit.cte}</span>}{edit.minuta && <span className="rounded-full bg-primary/10 px-2 py-1 text-primary">Minuta: {edit.minuta}</span>}</div><div className="mt-3 grid gap-3 border-t pt-3 sm:grid-cols-2"><div><Label>CTe</Label><Input value={edit.cte} onChange={(event) => editTrip(trip.id, "cte", event.target.value)} /></div><div><Label>Minuta</Label><Input value={edit.minuta} onChange={(event) => editTrip(trip.id, "minuta", event.target.value)} /></div></div></div></div></div> })}</div>}</CardContent></Card>}
