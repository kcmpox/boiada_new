...
import { useSlaughterhouses, useTrips, useTrucks, useFuelings, useExpenses, useDeductions, useReimbursements } from "@/lib/storage"
...
export function AlternativeLayoutDialog({ open, title, onBack }: { open: boolean; title: string; onBack: () => void }) {
  ...
  const jsonValue = { date: paymentDate, truckId, destination, tripIds: selectedTrips.map((trip) => trip.id), trips: selectedTrips.map((trip) => ({ ...trip, cte: tripEdits[trip.id]?.cte ?? trip.cte, minuta: tripEdits[trip.id]?.minuta ?? trip.minuta })), grossValue, rentValue, reimbursedValue: 0, expectedValue, receivedValue: Number(receivedValue.replace(",", ".")) || 0, notes }
